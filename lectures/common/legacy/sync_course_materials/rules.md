...
Вам нужно форкнуть текущий репозиторий к себе. Необходимо сделать его приватным и выдать доступы для преподавателей (@tonyblock, @andrei.kiselev, @Alpensin, @vgoryachkin, @g.gusev)
и менторов (@vilinvil, @DrepStep, @KonstantinGalanin, @Lalipopp4, @Papaloopalous, @samarec, @kissel, @nlfop, @p1cofly, @Heater, @wlcmtunknwndth, @nikolaypopov, @Daniil-Solo, @IvanTvardovsky, @xanderkhrenov) с уровнем доступа maintainer.
...
1. На выполнение и сдачу домашнего задания даётся три недели. Дедлайном является 10:00:00 субботы.
